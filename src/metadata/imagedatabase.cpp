    ...
    QObject(parent)
{
    ...
    if ( !drivers.contains("QSQLITE") ) {
        ...
        mDB = new QSqlDatabase(QSqlDatabase::addDatabase("QSQLITE","imagedatabase"));
        mDB->setDatabaseName(mDBFilePath);
        if ( !mDB->open() ) {
            qDebug() << "Imagedatabase not opened";
        } else {
            qDebug() << "DB opened";
        }
        createTables();
    ...
    connect(this,SIGNAL(requestArtistDownload(QString)),mDownloader,SLOT(requestArtistArt(QString)));
}
...
void ImageDatabase::albumReady(AlbumInformation *albumInformation) {
    ...
    if ( mAlbumNo < mAlbums->size() ) {
        mAlbumNo++;
        Albumtype album = mAlbums->at(mAlbumNo);
...
void ImageDatabase::fillDatabase(QMap<QString, QList<Albumtype>* > *map)
{
    qDebug() << "Received artist/album map from netaccess, start bulk download";
    foreach (QList<Albumtype> *albumList, *map ) {
        foreach (Albumtype album, *albumList) {
            int imageID = imageIDFromAlbumArtist(album.album,album.artist);
            if ( imageID == -1 ) {
    ...
    // Cleanup memory
    QMap<QString, QList<Albumtype>* >::iterator  iterator = map->begin();
...
void ImageDatabase::enterAlbumInformation(AlbumInformation *info)
{
    ...
    // If send ready URL
    if ( info->getName() == mCoverAlbum.album ) {
...
void ImageDatabase::enterArtistInformation(ArtistInformation  *info)
{
    ...
    // Check if this request was for currently playing title
    // If send ready URL
    if ( info->getArtist() == mCoverArtist ) {
...
QPixmap ImageDatabase::getAlbumImage(QString album, QString artist, bool download)
{
    ...
    if ( download ) {
        ...
        if ( mDownloadEnabled ) {
            emit requestAlbumDownload(albumobj);
        }
    }
    return QPixmap();
}
...
void ImageDatabase::requestCoverImage(Albumtype album)
{
    mCoverAlbum = album;
    qDebug() << "get Image for: " << album.album << album.artist;
    int artworkID = imageIDFromAlbumArtist(album.album,album.artist);
    ...
    if (artworkID >= 0 ) {
        ...
        artworkID = imageIDFromAlbum(album.album);
        if ( artworkID >= 0 ) {
            QString url = "image://"  IMAGEPROVIDERNAME  "/albumid/" + QString::number(artworkID);
            emit coverAlbumArtReady(QVariant::fromValue(url));
            return;
...
void ImageDatabase::requestCoverArtistImage(QString artist)
{
    mCoverArtist = artist;
    qDebug() << "get Image for: " << artist;
    int artworkID = imageIDFromArtist(artist);
...
void ImageDatabase::fillDatabase(QList<QString> *artistList)
{
    for(int i = 0; i < artistList->size(); i++) {
        QString artist = artistList->at(i);
        int imgID = imageIDFromArtist(artist);
        if(imgID == -1 ) {
            // Check if downloading is enabled first
            if ( mDownloadEnabled ) {
                qDebug() << "Start downloading info for: " << artist;
                emit requestArtistDownload(artist);
            }
        }
    }
